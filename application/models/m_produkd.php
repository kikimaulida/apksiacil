...
class m_produkd extends CI_Model {
	public function tampil_produk()
	{
		if (isset($_POST['nama_kecamatan']) AND isset($_POST['nama_kategori']))
		{
			...
			$array = implode(',',$data);
			$dt = $this->db->query("SELECT tb_produk.*, tb_usaha.id_kecamatan, tb_usaha.id_kategori, tb_kecamatan.nama_kecamatan, tb_kategori.nama_kategori FROM tb_produk JOIN tb_usaha ON tb_usaha.id_usaha = tb_produk.id_usaha JOIN tb_kecamatan ON tb_usaha.id_kecamatan = tb_kecamatan.id_kecamatan JOIN tb_kategori ON tb_usaha.id_kategori = tb_kategori.id_kategori WHERE tb_kecamatan.nama_kecamatan in($array) and tb_kategori.nama_kategori in($array1)")->result();
			return $dt;
		}
		...
		else if(isset ($_POST['nama_kecamatan']))
		{
			...
			$array = implode(',',$data);
			$dt = $this->db->query("SELECT tb_produk.*, tb_usaha.id_kecamatan, tb_kecamatan.nama_kecamatan FROM tb_produk JOIN tb_usaha ON tb_usaha.id_usaha = tb_produk.id_usaha JOIN tb_kecamatan ON tb_usaha.id_kecamatan = tb_kecamatan.id_kecamatan  WHERE tb_kecamatan.nama_kecamatan in($array)")->result();
			return $dt;
		}
